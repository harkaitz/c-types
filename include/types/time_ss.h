...
time_day_parse(time_t *_t, const char _s[], const char **_reason) {
    ...
    char *s_d = strtok_r(0, "-/\n\r", &s_t);
    if (!s_y) { error_reason(_reason, "Missing year, shall be YYYY-MM-DD");  return false; }
    if (!s_m) { error_reason(_reason, "Missing month, shall be YYYY-MM-DD"); return false; }
    if (!s_d) { error_reason(_reason, "Missing day, shall be YYYY-MM-DD");   return false; }
    long y = strtol(s_y, NULL, 10);
    long m = strtol(s_m, NULL, 10);
    long d = strtol(s_d, NULL, 10);
    if (y<100) y+=2000;
    if (y<1979      ) { error_reason(_reason, "Invalid year");  return false; }
    if (m<=0 || m>12) { error_reason(_reason, "Invalid month"); return false; }
    if (d<=0 || d>31) { error_reason(_reason, "Invalid day");   return false; }
    struct tm t = {0};
    t.tm_mday = d;
    t.tm_mon  = m;
    t.tm_year = y-1900;
